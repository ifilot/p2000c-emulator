...
enum { ES = 0, CS, SS, DS };
enum { CF = 0x0001, PF = 0x0004, AF = 0x0010, ZF = 0x0040,
       SF = 0x0080, TF = 0x0100, IF = 0x0200, DF = 0x0400,
       OF = 0x0800 };
enum { INT0_DIV_ERROR = 0, INT3_BREAKPOINT = 3, INT4_OVERFLOW = 4 };
...
struct i8088 {
    ...
    bool repeating;
    int rep;
    i8088_callbacks callbacks;
};

static thread_local i8088* active;

class ActiveCpu {
  public:
    explicit ActiveCpu(i8088* cpu) : previous_(active) { active = cpu; }
    ~ActiveCpu() { active = previous_; }

  private:
    i8088* previous_;
};

#define registers active->registers
#define byteRegisters active->byteRegisters
#define useMemory active->useMemory
#define address active->address
#define ip active->ip
#define opcode active->opcode
#define flags active->flags
#define modRM active->modRM
#define segment active->segment
#define segmentOverride active->segmentOverride
#define wordSize active->wordSize
#define sourceIsRM active->sourceIsRM
#define data active->data
#define destination active->destination
#define source active->source
#define residue active->residue
#define aluOperation active->aluOperation
#define savedIP active->savedIP
#define savedCS active->savedCS
#define running active->running
#define prefix active->prefix
#define repeating active->repeating
#define rep active->rep
...
static Word readWord(Word offset, int seg);
...
static void runtimeError(const char*, Args...)
{
    active->faulted = true;
}

static void performInterrupt(int intno)
{
    push(flags);
    push(cs());
    push(ip);
    ...
    if (!savedIP && !savedCS)
        runtimeError("INT vector not set");
    farJump();
}

static void divideOverflow(void)
{
    performInterrupt(INT0_DIV_ERROR);
    data = source = 1;
}

static DWord physicalAddress(Word offset, int seg, int)
{
    Word segmentAddress;
    DWord a;
...
static Byte readByte(Word offset, int seg)
{
    DWord a = physicalAddress(offset, seg, false);
    return active->callbacks.read(active->callbacks.context, a);
}

static Word readWord(Word offset, int seg)
{
    Word result = readByte(offset, seg);
    return result | (readByte(static_cast<Word>(offset + 1), seg) << 8);
}

static void writeByte(Byte value, Word offset, int seg)
{
    DWord a = physicalAddress(offset, seg, true);
    active->callbacks.write(active->callbacks.context, a, value);
}

static void writeWord(Word value, Word offset, int seg)
{
    writeByte(static_cast<Byte>(value), offset, seg);
    writeByte(static_cast<Byte>(value >> 8),
              static_cast<Word>(offset + 1), seg);
}
...
static Word fetch(bool fetchWordValue)
{
    if (fetchWordValue)
        return fetchWord();
    return fetchByte();
}
static Word signExtend(Byte value) {
    return value + (value < 0x80 ? 0 : 0xff00);
}
...
static void jumpShort(Byte displacement, bool jump)
{
    if (jump)
        doJump(ip + signExtend(displacement));
}
static void setCF(bool cf) { flags = (flags & ~1) | (cf ? 1 : 0); }
...
static void stoS(Word value)
{
    address = di();
    setDI(di() + stringIncrement());
    writewb(value, address, ES);
}
#define o(c)
/***void o(char c)
{
...
static void setCA() { setCF(true); setAF(true); }
...
static void finishWriteEA(Word value)
{
    if (!useMemory) {
        if (wordSize)
            registers[address] = value;
        else
            *byteRegisters[address] = (Byte)value;
    }
    else
        writewb(value, address, -1);
}
static void writeEA(Word value) { ea(); finishWriteEA(value); }
static void farLoad()
{
    if (!useMemory)
...
static void call(Word target) { push(ip); doJump(target); }
...
static void executeInstruction(void)
{
    ...
    switch (opcode) {
            ...
            case 0xdc: case 0xdd: case 0xde: case 0xdf:  // 8087 escape
                // With no 8087 installed, the 8088 still consumes the
                // instruction's effective-address bytes while the coprocessor
            ...
            case 0xe4: case 0xe5:   // IN ib
                data = active->callbacks.port_in(
                    active->callbacks.context, fetchByte(), wordSize);
                setAccum();
                break;
            case 0xe6: case 0xe7:   // OUT ib
                active->callbacks.port_out(
                    active->callbacks.context, fetchByte(), getAccum(),
                    wordSize);
                break;
            case 0xec: case 0xed:   // IN dx
                data = active->callbacks.port_in(
                    active->callbacks.context, dx(), wordSize);
                setAccum();
                break;
            case 0xee: case 0xef:   // OUT dx
                active->callbacks.port_out(
                    active->callbacks.context, dx(), getAccum(), wordSize);
                break;
            ...
            case 0xc6: case 0xc7:  // MOV rmv,iv
                ...
                break;
            case 0xcc:  // INT 3
                performInterrupt(INT3_BREAKPOINT);
                break;
            case 0xcd:
                performInterrupt(fetchByte());
                o('$');
                break;
            case 0xce:  // INTO
                if (of())
                    performInterrupt(INT4_OVERFLOW);
                break;
            case 0xcf:  // IRET
                o('I');
                doJump(pop());
...
extern "C" i8088* i8088_create(i8088_callbacks callbacks)
{
    if (!callbacks.read || !callbacks.write || !callbacks.port_in ||
        !callbacks.port_out)
        return nullptr;
    i8088* cpu = new (std::nothrow) i8088{};
    if (!cpu)
        return nullptr;
...
extern "C" void i8088_destroy(i8088* cpu)
{
    delete cpu;
}

extern "C" void i8088_reset(i8088* cpu)
{
    if (!cpu)
        return;
    const i8088_callbacks callbacks = cpu->callbacks;
    memset(cpu, 0, sizeof(*cpu));
    cpu->callbacks = callbacks;
    ActiveCpu guard(cpu);

    cpu->registers[1] = 0x00ff;
    Byte* byteData = reinterpret_cast<Byte*>(&cpu->registers[0]);
    const int bigEndian = byteData[2] == 0 ? 1 : 0;
    const int byteNumbers[8] = {0, 2, 4, 6, 1, 3, 5, 7};
    for (int index = 0; index < 8; ++index)
        cpu->byteRegisters[index] =
            &byteData[byteNumbers[index] ^ bigEndian];

    memset(cpu->registers, 0, sizeof(cpu->registers));
    cpu->registers[9] = 0xffff;
    cpu->flags = 0x0002;
    cpu->segmentOverride = -1;
    cpu->running = true;
}

extern "C" void i8088_step(i8088* cpu)
{
    if (!cpu || !cpu->running || cpu->halted || cpu->faulted)
        return;
    ActiveCpu guard(cpu);
    executeInstruction();
}

extern "C" bool i8088_interrupt(i8088* cpu, uint8_t vector)
{
    if (!cpu || !cpu->running || cpu->faulted || !(cpu->flags & IF))
        return false;
    ActiveCpu guard(cpu);
    cpu->halted = false;
    performInterrupt(vector);
    return !cpu->faulted;
}

extern "C" bool i8088_halted(const i8088* cpu)
{
    return cpu && cpu->halted;
}

extern "C" bool i8088_faulted(const i8088* cpu)
{
    return !cpu || cpu->faulted;
}

extern "C" uint16_t i8088_ip(const i8088* cpu)
{
    return cpu ? cpu->ip : 0;
}

extern "C" uint16_t i8088_cs(const i8088* cpu)
{
    return cpu ? cpu->registers[9] : 0;
}
